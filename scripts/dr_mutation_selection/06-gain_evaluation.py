...
Incremental gain ranking of EvoResist candidates relative to a shared baseline.

Baseline definition:
    intersection of List1 (WHO G1+G2) and List2 (EvoResist final list),
    i.e. variants present in both lists.

Candidates:
    variants in List2 that are absent from List1 (List2 - List1).
...
def load_all_snp_files(id_df, snp_dir):
    ...
    for _, row in id_df.iterrows():
        ...
        snp_df = pd.read_csv(path, sep="\t", header=None,
        ...
        snp_dict[id_] = set(
            zip(snp_df["position"], snp_df["ref"], snp_df["alt"]))
...
def load_all_indel_files(id_df, indel_dir):
    ...
    for _, row in id_df.iterrows():
        ...
        indel_df = pd.read_csv(path, sep="\t", header=None,
                               names=["position", "ref", "alt"],
                               usecols=[0, 1, 2], dtype=str)
        ...
        indel_dict[id_] = set(
            zip(indel_df["position"], indel_df["ref"], indel_df["alt"]))
...
def load_variant_list(filepath, label):
    ...
    with open(filepath) as fh:
        first_line_cols = [c.strip()
                           for c in fh.readline().rstrip("\n").split("\t")]
    ...
    if missing_cols:
        print(
            f"ERROR: {label} file is missing required columns: {sorted(missing_cols)}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    list1_set = set(
        zip(list1_df["position"], list1_df["ref"], list1_df["alt"]))
    list2_set = set(
        zip(list2_df["position"], list2_df["ref"], list2_df["alt"]))

    # Load IDs and genotype files
    id_df = load_id_list_new(args.id_list_file)
    snp_dict = load_all_snp_files(id_df, args.snp_dir)
    indel_dict = load_all_indel_files(id_df, args.indel_dir)
    ...
    S0, Spec0, PPV0 = compute_metrics(
        baseline_pred, actual_R, actual_S, universe)
    ...
    for v in candidates:
        ...
        S_v, Spec_v, PPV_v = compute_metrics(
            new_pred, actual_R, actual_S, universe)

        # Number of R isolates newly called by this variant beyond the baseline
        inc_R = len((variant_to_ids.get(v, set()) & actual_R) -
                    (baseline_pred & actual_R))
