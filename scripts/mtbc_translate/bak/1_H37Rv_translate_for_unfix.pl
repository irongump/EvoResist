...
foreach $i(@gene){
		if(($a[0]>=$start{$i})&&($a[0]<=$end{$i})){   # mutation location
			if($i=~m/MTB/){
				print "$a[0]\t$a[2]\t$a[3]\t$a[4]\t-\t-\t---\t---\t$i\t$name{$i}\t$des{$i}\t$cat{$i}";
				}elsif($k==0){
				if($strand{$i} eq "+"){
...
print "$a[0]\t$a[2]\t$a[3]\t$a[4]\t$codon\t$atype\t$code{$wd}-$code{$mt}\t$wd-$mt\t$i\t$name{$i}\t$des{$i}\t$cat{$i}";
				}
			if($k==1){
				if($strand{$i} eq "+"){
					$length=$end{$i}-$start{$i}+1;
...
$atype="nonsynonymous";
				...
				print "\t$codon\t$atype\t$code{$wd}-$code{$mt}\t$wd-$mt\t$i\t$name{$i}\t$des{$i}\t$cat{$i}";
				}
			$k++;		
			}
		}
		...
		foreach $j(@igr){
			if(($a[0]>=$start{$j})&&($a[0]<=$end{$j})){
				...
				$right=$start{$b[1]}-$a[0];
$atype="IGR";
				print "$a[0]\t$a[2]\t$a[3]\t$a[4]\t-\t$atype\t---\t$strand{$b[0]}$left-$right$strand{$b[1]}\t$j\t$name{$b[0]}-$name{$b[1]}\t$des{$b[0]}##$des{$b[1]}\t$cat{$b[0]}##$cat{$b[1]}";
				}
			}
		print "\n";	
}
