...
while(<F4>){
	...
	foreach $i(@gene){
		if(($a[0]>=$start{$i})&&($a[0]<=$end{$i})){   # mutation location
			if($i=~m/MTB/){
				print "$a[0]\t$a[1]\t$a[3]\t$i\t$name{$i}\t$start{$i}:$end{$i}\t$a[4]\t$des{$i}\t$cat{$i}\t$a[5]";
				}elsif($k==0){
				if($strand{$i} eq "+"){
...
$type="Nonsynonymous";
				...
				print "$a[0]\t$a[1]\t$a[3]\t$i\t$name{$i}\t$codon\t$a[4]\t$des{$i}\t$cat{$i}\t$a[5]\t";
$type="";				
}
			if($k==1){
				if($strand{$i} eq "+"){
...
$type="Nonsynonymous";
				...
				print "\t$codon\t$i\t$name{$i}\t$des{$i}\t$cat{$i}";
$type="";				
}
			$k++;		
			}
		...
		foreach $j(@igr){
			if(($a[0]>=$start{$j})&&($a[0]<=$end{$j})){
				...
				$left=$a[0]-$end{$b[0]};
				$right=$start{$b[1]}-$a[0];
				print "$a[0]\t$a[1]\t$a[3]\t$j\t$name{$b[0]}-$name{$b[1]}\t$strand{$b[0]}$left-$right$strand{$b[1]}\t$a[4]\t$des{$b[0]}##$des{$b[1]}\t$cat{$b[0]}##$cat{$b[1]}\t$a[5]\t";
				}
			}
		print "\n";	
}
